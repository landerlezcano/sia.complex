...
class modelo_afiliado extends CI_Model {
    ...
    private function Perfil($id, $rol) { #CrearPerfil
        //$_POST['tipo_afiliado'] = 2;
        //$profile = array();
        $identificacion = $this->depurarVariable($_POST['keyword'], rand(100000, 99999999));
        $curriculo = $this->depurarVariable($_POST['curriculo'], 'Ninguno');
        $sexo = $this->depurarVariable($_POST['sexo'], 1);
        $civil = $this->depurarVariable($_POST['civil'], 1);
        $estudios = $this->depurarVariable($_POST['estudios'], 1);
        $ocupacion = $this->depurarVariable($_POST['ocupacion'], 1);
        $tiempo_dedicado = $this->depurarVariable($_POST['tiempo_dedicado'], 1);
        $fiscal = $this->depurarVariable($_POST['fiscal'], 1);
        $nombre = $this->depurarVariable($_POST['nombre'], 'user_' . $id);
        $apellido = $this->depurarVariable($_POST['apellido'], 'last_' . $id);
        $nacimiento = $this->depurarVariable($_POST['nacimiento'], '1990-01-01');
        ...
        $dato_profile = array(
            ...
            /* "id_tiempo_dedicado" => */ $tiempo_dedicado,
            /* 'id_estatus'         => */ 1,
            /* "id_fiscal"       	 => */ $fiscal,
            /* "keyword"            => */ $identificacion,
            /* "paquete"             => $_POST['tipo_plan'] */ $curriculo,
            /* "nombre"             => */ $nombre,
            /* "apellido"           => */ $apellido,
    ...
    private function Coaplicante($id) { #CrearCoaplicante
        ...
        $nombre = $this->depurarVariable($_POST['nombre_co'], ' ');
        $apellido = $this->depurarVariable($_POST['apellido_co'], ' ');
        $contacto = $this->depurarVariable($_POST['keyword_co'], ' ');
    ...
    function afiliar_procedure(
        ...
        $procedure = 'CALL afiliar('
                . $id . ',"'
                . $dato_perfil . '",'
                . $dashboard . ',"'
                . $dato_estilo . '","'
                . $dato_coaplicante . /* '","'.$dato_tels. */'","'
                . $dato_dir . '","'
                . $dato_billetera . /* '","'.$dato_rango. */'","'
                . $dato_img
                . '")';
    ...
    function crearUsuario() {
        ...
        #$mi_red=$_POST['red'];		
        $rol = $_POST['rol'];
        $dashboard = $this->general->get_tipo_usuario_ID_Atributo($rol, "FK_perfil");
        ...
        if (isset($fijos) && isset($moviles))
            $this->insert_dato_tels($id, $fijos, $moviles);

        if (isset($sede) && isset($jefe))
            $this->insert_organizacion($sede, $id, $jefe);

        $dato_perfil = $this->Perfil($id, $rol); # USER_PROFILES
    ...
    private function dato_dir($id) {#insert_dato_dir
        $pais = $this->depurarVariable($_POST['pais'], 'MEX');
        $dato_dir = array(
            /* "id_user"   => */$id,
            /* "cp"        => */ $_POST['cp'],
            /* "calle"     => */ $_POST['calle'],
    ...
    private function insert_organizacion($sede, $id, $jefe) {

        $cargo = $this->depurarVariable($_POST['cargo'], 'Empleado');
        ...
        $this->db->insert("organizacion", $dato);
    ...
    function depurarVariable($var, $else) {
        return (strlen($var) > 1) ? $var : $else;
    }
