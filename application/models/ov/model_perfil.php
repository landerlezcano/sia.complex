...
class model_perfil extends CI_Model {
	...
	function datos_perfil($id) {
                $query =  'SELECT 
                                    ...
                                    p.id_estudio id_estudio,
                                    p.id_ocupacion id_ocupacion,
                                    p.id_tiempo_dedicado id_tiempo_dedicado,
                                    p.id_fiscal id_fiscal,
                                    p.curriculo,
                                ...
                                FROM
                                    user_profiles p
                                        LEFT JOIN
                                    cat_sexo sexo ON p.id_sexo = sexo.id_sexo
                                        LEFT JOIN
                                    estilo_usuario estilos ON p.user_id = estilos.id_usuario
                                        LEFT JOIN
                                    cat_edo_civil edo_civil ON p.id_edo_civil = edo_civil.id_edo_civil
                                WHERE
                                    p.user_id = ' . $id;
...
order by (U.id);" );
	...
	function use_mail_modificar() {
                $query = "select * from users where lower(email) like '" . strtolower ( $_POST ['mail'] ) . "' and id != '" . $_POST ['id'] . "'";
	...
	function edo_civil() {
        ...
        function update_dir($id,$pais){
            ...
            $dato_dir = array (
				...
				"calle" => $_POST ['calle'] ? $_POST ['calle'] : 1,
				"colonia" => $_POST ['colonia'] ? $_POST ['colonia'] : 1,
				"municipio" => $_POST ['municipio'] ? $_POST ['municipio'] : 1,
				"estado" => $_POST ['estado'] ? $_POST ['estado'] : 1,
        ...
        function update_estilo($id){
            ...
            $this->db->update ( "estilo_usuario", $dato, "id_usuario=". $id );
        ...
        function update_perfil($id){
            ...
            $dato = array(
              'id_sexo' => $_POST ['sexo'] ? $_POST ['sexo'] : 1,
              'id_fiscal' => $_POST ['fiscal'] ? $_POST ['fiscal'] : 1,              
              'id_edo_civil' => $_POST ['civil'] ? $_POST ['civil'] : 1,
              'id_estudio' => $_POST ['estudios'] ? $_POST ['estudios'] : 1,
              'id_ocupacion' => $_POST ['ocupacion'] ? $_POST ['ocupacion'] : 1,
              'id_tiempo_dedicado' => $_POST ['tiempo_dedicado'] ? $_POST ['tiempo_dedicado'] : 1,
              'keyword' => $_POST ['keyword'] ? $_POST ['keyword'] : 1,
              'curriculo' => $_POST ['curriculo'] ? $_POST ['curriculo'] : 1,
              'nombre' => $_POST ['nombre'],
              'apellido' => $_POST ['apellido'],
              'fecha_nacimiento' => $_POST ['nacimiento'],
            ); 
	...
	function actualizar($id, $pais) {
            ...
            $this->update_dir($id, $pais); 
            
            $this->update_user($id); 
               
            $this->update_perfil($id);  
            
            //$this->update_estilo($id);
	...
	function img_user_name($id, $data) {
 ...
 where   u.user_id=" . $id_user );
	...
	function eliminar($id) {
		...
		$this->db->query ( "delete from organizacion where usuario not in (select id from users)" );
