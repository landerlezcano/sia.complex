...
class model_admin extends CI_Model
{
	...
	function kill_afiliado($id,$red){
		...
		$i=0;
		$redes_afiliado = $this->model_perfil->ConsultarRedAfiliado($id);	
		if ($red==0){
	...
	function flowCompress($id,$red){
		...
		$hijos = $this->model_perfil->ConsultarHijos($id,$red);
		$lados = $this->model_tipo_red->ObtenerFrontalesRed($red);
		$espacio = ($hijos) ? $this->buscarEspacios($id,$red,$lados[0]->frontal,count($hijos)) : 2;
		//echo "padre: ".$espacio."	";
		$setHijos = $this->model_perfil->ConsultarRedDebajo($id,$red);
		$failure = ($hijos) ? $this->model_perfil->actualizarHijos($id,$espacio,$setHijos[0]->hijos,$red,$hijos) : true;
		return $failure;
	...
	function buscarEspacios($id,$red,$espacios,$cupos){
		...
		$padre = $this->model_perfil->ConsultarPadre($id , $red);
		$frontales = (count($this->model_perfil->ConsultarHijos($padre,$red))-1);	
	...
	function rotarPadres($espacios,$padre,$frontales,$cupos,$red){
		...
		while ($padre<>2){
			...
			if (($frontales + $cupos)<= $espacios){			
				...
				$padre = $this->model_perfil->ConsultarPadre($padre , $red);
				$frontales = count($this->model_perfil->ConsultarHijos($padre,$red));
			}
		}
		//echo "padre : ".$padre." ";
		return $padre;
