...
class cursos extends CI_Controller
{
	...
	function index()
	{
		...
		$this->template->build('website/bo/cursos/index');
	}
	...
	function alta()
	{
		...
		$this->template->set_partial('header', 'website/bo/header');
		$this->template->set_partial('footer', 'website/bo/footer');
		$this->template->build('website/bo/cursos/alta');
	}
	...
	function editar()
	{
		...
		$curso = $this->modelo_cursos->consultar($id);
		#$galeria = $this->modelo_cursos->consultarGaleria ( $id );
		...
		$this->template->build('website/bo/cursos/editar');
	}
		...
		$cursos = $this->modelo_cursos->listar();
		#$sedes = $this->modelo_sede->consultar_ACT();
		...
		$this->template->set_partial('header', 'website/bo/header');
		$this->template->set_partial('footer', 'website/bo/footer');
		$this->template->build('website/bo/cursos/listar');
	}
		...
		$id = $this->modelo_cursos->crear($curso);
          echo   ($id)  ? "El Curso ha sido creado satisfactoriamente"
                : "El Curso no pudo crearse. Por favor, verifique los datos e intente de nuevo"; 
	...
	function eliminar(){
		...
		$this->modelo_cursos->eliminar($id);
		echo  'El Curso ha sido eliminado corectamente';
	}
	...
	function cambiar_estado(){
		...
		$this->modelo_cursos->cambiar_estado($id,$estado);
	...
	function actualizar() {
		...
		$retorno = $this->modelo_cursos->actualizar($curso,$id);
		echo   ($retorno)  ? "El Curso ha sido Actualizado satisfactoriamente"
				: "El Curso no pudo crearse. Por favor, verifique los datos e intente de nuevo";
	...
	function actualizarWeb($sede){
		...
		$datos = $this->modelo_cursos->consulta_sede($sede->id);
	...
	private function setWebCursos($datos) {
		...
		foreach ($datos as $dato){
			...
			$image = $this->modelo_cursos->consultarGaleria($dato->id);
	...
	private function setDataCurso($dato,$i,$image) {
		...
		$tabla = 'var div'.$i.' = $("<div/>",  { "class": "moduletable" , "css": {"border" : "thin solid #000", "background" : "#f1f1f1"} });
				$("#modal-'.$i.' .modal-body").html(div'.$i.');
				  div'.$i.' = $("<div/>",  { "class": "row-fluid sp-pricing" });
				$("#modal-'.$i.' .moduletable").append(div'.$i.');
				  div'.$i.' = $("<div/>",  { "class": "span12" });
				$("#modal-'.$i.' .row-fluid.sp-pricing").append(div'.$i.');
				  div'.$i.' = $("<div/>",  { "class": "plan" });
				$("#modal-'.$i.' .span12").append(div'.$i.');
				  div'.$i.' = $("<div/>",  { "class": "plan-img" , "css": {"text-align" : "center"} });
				$("#modal-'.$i.' .plan").append(div'.$i.');
				  div'.$i.' = $("<img/>",  { "src": "'.$image.'" , "height": "200px" });
				$("#modal-'.$i.' .plan-img").append(div'.$i.');
				  div'.$i.' = $("<div/>",  { "class": "plan-name" , "css": {"color" : "#fff", "background" : "#4d4d4d", "text-align" : "center"} });
				$("#modal-'.$i.' .plan").append(div'.$i.');
				$("#modal-'.$i.' .plan-name").html("<h1>'.ucfirst($dato->nombre).'</h1>");
				  div'.$i.' = $("<div/>",  { "class": "plan-price" , "css": {"text-align" : "center"} });
				$("#modal-'.$i.' .plan").append(div'.$i.');
				$("#modal-'.$i.' .plan-price").html("<pre>Inscripcion: <strong>'.$dato->costo.'</strong> </pre>");				  
				  div'.$i.' = $("<div/>",  { "class": "plan-date" , "css": {"text-align" : "center"} });
				$("#modal-'.$i.' .plan").append(div'.$i.');
				$("#modal-'.$i.' .plan-date").html("<h2>De '.$dato->inicio.' a '.$dato->fin.'</h2>");
				  div'.$i.' = $("<div/>",  { "class": "plan-doc" , "css": {"text-align" : "center"} });
				$("#modal-'.$i.' .plan").append(div'.$i.');
				 $("#modal-'.$i.' .plan-doc").html("<h3><strong>Profesor:</strong> '.ucfirst($dato->docente).'</h3>");
				  div'.$i.' = $("<div/>",  { "class": "plan-desc" , "css": {"text-align" : "center"} });
				$("#modal-'.$i.' .plan").append(div'.$i.');
				$("#modal-'.$i.' .plan-desc").html("<pre>'.ucfirst($dato->descripcion).'</pre>");';
	...
	private function executeWebSH($host, $user, $pass, $db, $data) {
		$phpEXE = "sh";
