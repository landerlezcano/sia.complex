...
class perfil extends CI_Controller
{
	...
	function index()
	{
		...
		$this->template->build('website/sede/perfil/index');
	}
  	...
  	function datos()
	{
		...
		$this->template->build('website/sede/perfil/perfil');
	}
	...
	function get_red_afiliar()
	{
		...
		if($frontalidadRed==$INFINITO){
			$frontalesUsuario = $this->model_perfil->get_cantidad_de_frontales($id_afiliado,$id_red);
	...
	private function printRedParaAfiliar($id_red,$id_afiliado, $frontales,$nivel) {
		...
		for($lado=0;$lado<$frontales;$lado++){
			$afiliado = $this->model_perfil->get_afiliado_por_posicion($id_red,$id_afiliado,$lado);
			
			if($afiliado){
				$this->printPosicionAfiliado ( $nivel, $afiliado);
			}else {
				$sponsor=$this->model_perfil->get_name($id_afiliado);
	...
	private function setImagenAfiliado($id_afiliado) {
		$image 			 = $this->model_perfil->get_images($id_afiliado);
	...
	function subtree()
	{
		...
		$afiliados = $this->model_perfil->get_afiliados($id_red, $_POST['id']);
		$frontales=count($afiliados);
		
		$nombre=$this->model_perfil->get_name($_POST['id']);
		...
		if($afiliados)
		{
			...
			{
				$usuario[]=$this->model_perfil->datos_perfil($id_afiliado->id_afiliado);
			}
			...
			foreach ($usuario as $afiliado)
			{
				...
				$image 			 = $this->model_perfil->get_images($afiliado[0]->user_id);
				$img_perfil='/template/img/empresario.jpg';
				foreach ($image as $img)
				{
					$cadena=explode(".", $img->img);
				...
				else
				{
					...
					$sponsor 			 = $this->model_perfil->get_sponsor_id($afiliado[0]->user_id,$id_red);
	...
	
	{
		...
		$nivel = ($this->tank_auth->get_user_id()==2) ? 0 : $_POST['nivel'];
		$red 	 = $this->model_tipo_red->ObtenerFrontalesRed($id_red);
		$afiliados = $this->model_perfil->get_afiliados_directos($id_red, $_POST['id']);
	...
	function detalle_usuario()
	{
		...
		$image 			 = $this->model_perfil->get_images($_POST['id']);
		...
		$usuario =$this->model_perfil->datos_perfil($_POST['id']);
		$edad    =$this->model_perfil->edad($_POST['id']);
		$telefonos    =$this->model_perfil->telefonos($_POST['id']);
		$dir    =$this->model_perfil->dir($_POST['id']);
	...
	function validate_user_data()
	{
		...
		$use_mail=$this->model_perfil->use_mail();
		$use_username=$this->model_perfil->use_username();
	...
	function validate_user_data2() {
		...
		$use_mail=$this->model_perfil->use_mail_modificar();
		$use_username=$this->model_perfil->use_username_modificar();
	...
	{
		$use_mail_modificar = $this->model_perfil->use_mail_modificar();
	...
	{
		$use_keyword=$this->model_perfil->use_keyword();
		if($use_keyword)
		{
	...
	function use_username_red()
	{
		...
		$id_user=$this->model_perfil->get_id_by_username($_POST['username']);
		
		if(isset($id_user[0]->id)){
			$use_username_red=$this->model_perfil->use_username_red($id_user[0]->id);
			if(!$use_username_red){
				echo "";
				return true;
			}
	...
	{
		$use_username_modificar = $this->model_perfil->use_username_modificar();
	...
	function afiliar_nuevo()
	{
		...
		isset($_POST['token']) ? $this->model_perfil->trash_token($_POST['token']) : '';
	...
	function afiliar_nuevo_r($id)
	{
		sleep(3);
		//print_r($_POST);
		$this->model_perfil->afiliar_nuevo($id);
		if($_POST['sponsor'])
		{
			$id_=$this->tank_auth->get_user_id();
			$this->model_perfil->actualiza_directo($id_,$id);
		}
		$id_afiliado=$this->model_perfil->get_id();
	...
	function actualizar()
	{
		...
		$id=$this->tank_auth->get_user_id();
		$this->model_perfil->actualizar($id,$pais[0]->codigoPais);
	...
	function cp()
	{
		...
		{
			$busqueda=$this->model_perfil->cp();
			if($busqueda)
			{
				$id_estado=$busqueda[0]->id_estado;
				$estado=$this->model_perfil->estado($id_estado);
	...
	function foto()
	{
        ...
        $this->template->set_partial('header', 'website/sede/header');
        $this->template->set_partial('footer', 'website/sede/footer');
		$this->template->build('website/sede/perfil/foto');
	}
	...
	function sube_foto($tipo)
	{
		...
		else
		{
			...
			$this->model_perfil->img_user_name($id,$data["upload_data"]);
		}
	}
  	...
  	function sube_foto_tomar($tipo)
	{
		...
		{
			$this->model_perfil->img_user_tomar($id);
			echo '1';
		}
	}
	...
	function MensajeFase(){
		$id = $_POST['id'];
		$red = $_POST['red'];
		$valor_retencion  = $this->model_perfil->ObtenerRetencioFase();
		$this->template->set("id",$id);
